...
const AbrirCajaModal = dynamic(
  () => import('@/components/caja/AbrirCajaModal').then((m) => m.AbrirCajaModal),
  { ssr: false }
);
const MovimientoCajaModal = dynamic(
  () => import('@/components/caja/MovimientoCajaModal').then((m) => m.MovimientoCajaModal),
  { ssr: false }
);
const ArqueoCierreModal = dynamic(
  () => import('@/components/caja/ArqueoCierreModal').then((m) => m.ArqueoCierreModal),
  { ssr: false }
);
const ComprobanteArqueoModal = dynamic(
  () => import('@/components/caja/ComprobanteArqueoModal').then((m) => m.ComprobanteArqueoModal),
  { ssr: false }
);


export interface CajaInteractiveIslandProps {
  initialSesion?: any;
  initialResumen?: any;
  initialMovimientos?: any[];
  initialPagosEfectivo?: any[];
  initialHistorial?: any[];
}

export function CajaInteractiveIsland({
  initialSesion = null,
  initialResumen = null,
  initialMovimientos = [],
  initialPagosEfectivo = [],
  initialHistorial = []
}: CajaInteractiveIslandProps) {
  const { 
    sesionActiva, 
    isCajaAbierta, 
    resumenTurno, 
  ...
  useEffect(() => {
    if (!seededRef.current && (initialSesion || initialHistorial.length > 0)) {
      useCajaStore.setState({
        sesionActiva: initialSesion || null,
        isCajaAbierta: initialSesion?.estado === 'ABIERTA',
        resumenTurno: initialResumen || null,
        movimientos: initialMovimientos || [],
        pagosEfectivo: initialPagosEfectivo || [],
        historialSesiones: initialHistorial || [],
        isLoading: false
      });
  ...
  }, [cargarSesionActiva, cargarHistorial, initialSesion, initialResumen, initialMovimientos, initialPagosEfectivo, initialHistorial]);
