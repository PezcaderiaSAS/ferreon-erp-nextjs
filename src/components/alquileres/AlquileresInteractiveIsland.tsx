...
import {
  Plus, Search, MoreVertical, FileSpreadsheet, FileText,
  Printer, CheckCircle, HardHat, History,
  CheckCircle2, DollarSign, HelpCircle, ArrowLeftRight
} from "lucide-react";
...
} from '@/app/actions/alquileres';
import { registrarPagoAction } from '@/app/actions/pagos';

import { alquilerEntityToAlquilerUI, equipoToEquipoUI } from '@/lib/mappers';
...
// Dynamic client components for lazy bundle loading
...
const DetalleAlquilerModal = dynamic(
  () => import('@/app/components/alquileres/DetalleAlquilerModal').then((m) => m.DetalleAlquilerModal),
  { ssr: false, loading: () => <ModalSkeleton message="Cargando detalle 360°..." /> }
);

const TicketAlquilerModal = dynamic(
  () => import('@/app/components/alquileres/TicketAlquilerModal').then((m) => m.TicketAlquilerModal),
  { ssr: false, loading: () => <ModalSkeleton message="Preparando ticket de alquiler..." /> }
);

const AprobarCotizacionModal = dynamic(
  () => import('@/app/components/alquileres/AprobarCotizacionModal').then((m) => m.AprobarCotizacionModal),
...
const CrearCotizacionModal = dynamic(
  () => import('@/app/components/cotizaciones/CrearCotizacionModal').then((m) => m.CrearCotizacionModal),
  { ssr: false }
);

import { CicloVidaAlquilerBanner } from '@/app/components/alquileres/CicloVidaAlquilerBanner';
import { GuiaBotonesModal } from '@/app/components/alquileres/GuiaBotonesModal';

export type AlquilerTabType = 'contratos' | 'cotizaciones' | 'historial';

interface AlquileresInteractiveIslandProps {
  initialAlquileres: AlquilerUI[];
  initialCotizaciones: any[];
}

export function AlquileresInteractiveIsland({
  initialAlquileres,
  initialCotizaciones
}: AlquileresInteractiveIslandProps) {
  const { alquileres, setAlquileres, sanitizeStore } = useAlquilerStore();
  ...
  const [cotizacionesList, setCotizacionesList] = useState<any[]>(initialCotizaciones);
  ...
  // Contenedor de scroll y Virtualizador de Filas (60fps)
  const tableContainerRef = useRef<HTMLDivElement>(null);

  // Inicializar store con props del RSC si está vacío
  useEffect(() => {
    if (alquileres.length === 0 && initialAlquileres.length > 0) {
      setAlquileres(initialAlquileres);
    }
  }, [initialAlquileres, alquileres.length, setAlquileres]);

  // Deep linking de pestañas
  useEffect(() => {
      ...
      if (alq.estado === 'COTIZACION' || alq.estado === 'FORMALIZADA' || alq.tipo === 'COTIZACION' || (alq as any).tipoDocumento === 'COTIZACION') {
        ...
        if (!seenIds.has(idKey) && (!conKey || !seenIds.has(conKey))) {
          list.push({
            ...
            fecha_inicio: (alq as any).fecha_inicio || (alq as any).fechaInicio || alq.fecha_inicio_contrato,
            fechaInicio: (alq as any).fecha_inicio || (alq as any).fechaInicio || alq.fecha_inicio_contrato,
            fecha_fin_estimada: alq.fecha_fin_estimada || (alq as any).fechaFinEstimada,
            fechaFinEstimada: alq.fecha_fin_estimada || (alq as any).fechaFinEstimada,
  ...
  // Virtualización de filas para listas grandes (>25 contratos)
  const rowVirtualizer = useVirtualizer({
    count: alquileresFiltrados.length,
  ...
  const fetchAllData = useCallback(async () => {
    try {
      ...
      console.warn('[AlquileresIsland] Error cargando catálogos:', e);
    } finally {
      setLoading(false);
    }
  }, [setAlquileres]);
  ...
  const handleAprobarCotizacion = async (alquilerId: string, ajustes: any) => {
    try {
      ...
      alert("Cotización comercial aprobada y stock reservado correctamente.");
    } catch (error: any) {
          ...
          {activeTab === 'cotizaciones' ? (
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100 bg-white">
                {(() => {
                  const isVirtualized = alquileresFiltrados.length > 25;
                  ...
                  const paddingBottom = isVirtualized && virtualItems.length > 0
                    ? Math.max(0, rowVirtualizer.getTotalSize() - virtualItems[virtualItems.length - 1].start - virtualItems[virtualItems.length - 1].size)
                    : 0;
                  const itemsToRender = isVirtualized
                    ? virtualItems.map(v => alquileresFiltrados[v.index]).filter(Boolean)
                    : alquileresFiltrados;

                  return (
                    <>
                      {paddingTop > 0 && <tr><td colSpan={7} style={{ height: `${paddingTop}px` }} /></tr>}
                      {itemsToRender.map((alq: any) => (
                        <tr 
                          key={alq.id} 
                          onClick={() => {
                      ...
                      <div className="flex items-center justify-end gap-1.5" onClick={(e) => e.stopPropagation()}>
                        {alq.estado === 'ACTIVO' && (
                          <button
                            ...
                            title="Recibir devolución física de maquinaria"
                          >
                        ...
                        <button 
                          type="button"
                          onClick={() => toggleDropdown(String(alq.id))}
                          ...
                          aria-label={`Opciones para contrato #${alq.id}`}
                        >
                          <MoreVertical className="w-4 h-4" />
                        </button>
                      </div>
                      
                      {activeDropdown === String(alq.id) && (() => {
