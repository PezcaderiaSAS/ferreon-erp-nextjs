...
import { CompraUI } from '@/app/actions/compras';
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`Comprobante de Entrada: ${compra.numero_orden}`} maxWidth="4xl">
      <div className="p-6 space-y-6">
        ...
        {/* Hoja de ImpresiÃ³n EstÃ¡ndar (Carta / A4) */}
        <div id="comprobante-compra-print" className="bg-white p-8 border border-slate-200 rounded-2xl shadow-sm space-y-6 text-slate-800 print:border-none print:shadow-none print:p-0">
          ...
          <div className="flex justify-between items-start border-b border-slate-200 pb-6">
            <div>
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 font-mono">NIT: 901.884.215-9 | RÃ©gimen ComÃºn</p>
                </div>
              </div>
              <p className="text-xs text-slate-500 mt-2">
                Especialistas en Alquiler y Venta de Maquinaria para ConstrucciÃ³n<br />
                PBX: (601) 745-9000 | Calle 80 # 68-45, BogotÃ¡ D.C.
              </p>
            </div>
            ...
            <div className="text-right">
              <span className="inline-block px-3 py-1 bg-emerald-50 border border-emerald-200 text-emerald-800 font-bold text-xs rounded-lg uppercase tracking-wider">
                Orden de Compra / Entrada de AlmacÃ©n
              </span>
              <h3 className="text-2xl font-black font-mono text-slate-900 mt-2">
                {compra.numero_orden}
              </h3>
              <p className="text-xs text-slate-500 mt-1">
                Fecha de EmisiÃ³n: <span className="font-semibold text-slate-700">{compra.fecha_compra}</span>
              </p>
          ...
          {/* Datos del Proveedor y TÃ©rminos Comerciales */}
            ...
            <div>
              ...
              {compra.proveedor_telefono && (
                <p className="text-slate-600">
                  <span className="font-medium text-slate-500">TelÃ©fono:</span> {compra.proveedor_telefono}
                </p>
              )}
          ...
          {/* LiquidaciÃ³n Financiera y Tributaria */}
          <div className="flex justify-end pt-2">
            <div className="w-80 space-y-2 text-xs border-t border-slate-200 pt-3">
              ...
              {compra.aplica_reteica && (
                <div className="flex justify-between text-rose-600">
                  <span>(-) ReteICA ({compra.porcentaje_reteica || 9.66}â€°):</span>
          ...
          {/* Casillas de Firmas y ValidaciÃ³n */}
          <div className="grid grid-cols-3 gap-6 pt-12 text-xs text-center border-t border-slate-200">
            <div>
              ...
              <p className="text-2xs text-slate-400">Nombre, CÃ©dula y Firma</p>
            </div>
            <div>
              <div className="border-b border-slate-300 h-10 mb-2"></div>
              <p className="font-semibold text-slate-700">Recibido en Bodega</p>
              <p className="text-2xs text-slate-400">Jefe de AlmacÃ©n / Operador</p>
            </div>
            <div>
              <div className="border-b border-slate-300 h-10 mb-2"></div>
              <p className="font-semibold text-slate-700">AprobaciÃ³n Contable</p>
              <p className="text-2xs text-slate-400">AuditorÃ­a / TesorerÃ­a FerreOn</p>
            </div>
          </div>
        </div>
      </div>
