...
import { CompraUI, recibirMercanciaEnBodegaAction } from '@/app/actions/compras';
...
export function RecibirMercanciaModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validar que no se reciba mÃ¡s de lo comprado (Poka-Yoke)
    ...
    try {
      const obsTexto = [
        ...
        hayDiscrepancia ? `Cotejo fÃ­sico: ${totalUnidadesRecibir}/${totalUnidadesEsperadas} un.` : null
      ].filter(Boolean).join(' | ');
      ...
      if (res.success) {
        showSuccessToast('MercancÃ­a recibida en bodega y Costo Promedio Ponderado (PMP) actualizado con Ã©xito.');
        onRecepcionExitosa();
        onClose();
      } else {
        showErrorToast(res.error || 'Error al procesar la recepciÃ³n de mercancÃ­a.');
      }
    } catch (err: any) {
      console.error('[RecibirMercanciaModal] Error:', err);
      showErrorToast(err.message || 'OcurriÃ³ un error inesperado al recibir la mercancÃ­a.');
    } finally {
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={`RecepciÃ³n en Bodega â€” ${compra.numero_orden}`}
      maxWidth="3xl"
    >
      <form onSubmit={handleSubmit} className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-2xl text-xs">
          ...
          <div>
            <span className="text-slate-500 flex items-center gap-1 font-medium">
              <ShieldCheck className="w-3.5 h-3.5 text-emerald-500" /> ValuaciÃ³n Contable:
            </span>
        ...
        {/* NotificaciÃ³n Poka-Yoke de impacto en Kardex y PMP */}
        <div className="flex items-start gap-3 p-3.5 bg-blue-50/80 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-900 rounded-xl text-xs text-blue-800 dark:text-blue-300">
          ...
          <div>
            <span className="font-bold">Mecanismo Poka-Yoke & PMP AutomÃ¡tico:</span>
            <p className="mt-0.5 leading-relaxed">
              Al confirmar la recepciÃ³n fÃ­sica, el sistema recalcularÃ¡ en tiempo real el <strong>Costo Promedio Ponderado (PMP)</strong> de cada equipo segÃºn la fÃ³rmula oficial de inventarios, asentarÃ¡ los movimientos en el Kardex y aumentarÃ¡ el stock disponible en bodega.
            </p>
          </div>
        </div>

        {/* Tabla de VerificaciÃ³n FÃ­sica de Items */}
        <div className="space-y-2">
          <div className="flex justify-between items-center">
            <h4 className="text-xs font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300 flex items-center gap-1.5">
              <PackageCheck className="w-4 h-4 text-emerald-600" />
              Cotejo FÃ­sico de MercancÃ­a
            </h4>
            ...
            <table className="w-full text-left text-xs border-collapse">
              <thead className="bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 font-semibold border-b border-slate-200 dark:border-slate-800">
                <tr>
                  ...
                  <th className="p-3 text-center w-36">Cant. FÃ­sica Recibida</th>
          ...
          {hayDiscrepancia && (
            <p className="text-[11px] text-amber-600 dark:text-amber-400 flex items-center gap-1.5 font-medium mt-1">
              ...
              AtenciÃ³n: La cantidad recibida ({totalUnidadesRecibir}) difiere de la cantidad comprada ({totalUnidadesEsperadas}). La orden se registrarÃ¡ con recepciÃ³n parcial.
            </p>
          )}
        </div>

        {/* Metadatos de la Entrada (RemisiÃ³n y Bodega) */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">
              NÃºmero de GuÃ­a / RemisiÃ³n de Entrega
            </label>
            <input
              type="text"
            ...
            />
            <p className="text-[10px] text-slate-400 mt-1">Documento fÃ­sico del transportador o proveedor</p>
          </div>
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-3 py-2 text-xs rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500"
            >
              <option value="BODEGA_CENTRAL">Bodega Central (BogotÃ¡)</option>
              <option value="BODEGA_NORTE">Bodega Norte / Taller</option>
              <option value="EN_TRANSITO">En TrÃ¡nsito / Patio de Maniobras</option>
            </select>
            <p className="text-[10px] text-slate-400 mt-1">UbicaciÃ³n donde se almacenarÃ¡n las unidades</p>
          </div>
        </div>
        ...
        <label className="flex items-center gap-2.5 p-3 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-800 cursor-pointer">
          ...
          <span className="text-xs font-medium text-slate-700 dark:text-slate-300">
            Marcar la Orden de Compra como <strong>COMPLETADA</strong> tras esta recepciÃ³n fÃ­sica
          </span>
        </label>
