...
export function RegistrarAbonoProveedorModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success && res.data) {
        showSuccessToast('Abono asentado y comprobante de egreso generado con Ã©xito.');
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={`Registrar Abono / Pago a Proveedor â€” ${cuentaPagar.numero_orden}`}
      maxWidth="2xl"
    >
      <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            <div className="mt-1">
              {cuentaPagar.semaforo === 'VENCIDA' && (
                <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-rose-100 text-rose-800 dark:bg-rose-950 dark:text-rose-300">
                  Vencido ({cuentaPagar.dias_mora} dÃ­as)
                </span>
              )}
              {cuentaPagar.semaforo === 'POR_VENCER' && (
                <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-amber-100 text-amber-800 dark:bg-amber-950 dark:text-amber-300">
                  PrÃ³ximo a vencer ({cuentaPagar.dias_restantes} dÃ­as)
                </span>
              )}
              {cuentaPagar.semaforo === 'AL_DIA' && (
                <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-100 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-300">
                  Al dÃ­a ({cuentaPagar.dias_restantes} dÃ­as)
                </span>
              )}
            </div>
          </div>
        ...
        {/* Input de Monto con Accesos RÃ¡pidos Poka-Yoke */}
        ...
        {metodoPago === 'EFECTIVO' && (
          <div className="flex items-start gap-2.5 p-3 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900 rounded-xl text-xs text-amber-800 dark:text-amber-300">
            ...
            <div>
              <span className="font-bold">Efectivo / Caja:</span> Este egreso se vincularÃ¡ a la sesiÃ³n de caja abierta actualmente en el sistema y descontarÃ¡ del saldo fÃ­sico disponible.
            </div>
          </div>
        )}
        ...
        {metodoPago !== 'EFECTIVO' && (
          <div>
            <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">
              {metodoPago === 'TRANSFERENCIA' ? 'NÃºmero de AprobaciÃ³n / Comprobante Bancario' : 'NÃºmero de Cheque y Banco'}
            </label>
            <input
              type="text"
              placeholder={metodoPago === 'TRANSFERENCIA' ? 'Ej. REF-983419 Bancolombia' : 'Ej. CHQ-44910 Banco de BogotÃ¡'}
