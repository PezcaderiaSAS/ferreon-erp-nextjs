...
import { obtenerComprasAction, CompraUI } from '@/app/actions/compras';
...
const ComprobanteEntradaPDFModal = dynamic(
  () => import('@/components/compras/ComprobanteEntradaPDFModal').then((mod) => mod.ComprobanteEntradaPDFModal),
  { ssr: false, loading: () => <ModalSkeleton /> }
);
const CrearProveedorModal = dynamic(
  () => import('@/components/proveedores/CrearProveedorModal').then((mod) => mod.CrearProveedorModal),
  { ssr: false, loading: () => <ModalSkeleton /> }
);
const RecibirMercanciaModal = dynamic(
  () => import('@/components/compras/RecibirMercanciaModal').then((mod) => mod.RecibirMercanciaModal),
  { ssr: false, loading: () => <ModalSkeleton /> }
);
const RegistrarAbonoProveedorModal = dynamic(
  () => import('@/components/compras/RegistrarAbonoProveedorModal').then((mod) => mod.RegistrarAbonoProveedorModal),
  { ssr: false, loading: () => <ModalSkeleton /> }
);
const ComprobanteEgresoPDFModal = dynamic(
  () => import('@/components/compras/ComprobanteEgresoPDFModal').then((mod) => mod.ComprobanteEgresoPDFModal),
  { ssr: false, loading: () => <ModalSkeleton /> }
);

export interface ComprasInteractiveIslandProps {
  initialCompras?: CompraUI[];
  initialProveedores?: ProveedorUI[];
  initialCuentasPagar?: CuentaPagarUI[];
}

export function ComprasInteractiveIsland({
  initialCompras = [],
  initialProveedores = [],
  initialCuentasPagar = []
}: ComprasInteractiveIslandProps) {
  ...
  // Datos
  const [compras, setCompras] = useState<CompraUI[]>(initialCompras);
  const [proveedores, setProveedores] = useState<ProveedorUI[]>(initialProveedores);
  const [cuentasPagar, setCuentasPagar] = useState<CuentaPagarUI[]>(initialCuentasPagar);
